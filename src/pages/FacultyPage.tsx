    ...
    <div className="relative min-h-screen bg-background pb-mobile-nav pt-20 md:pt-24 px-4 md:px-8 lg:px-16 bg-geo-pattern">
      ...
      <div className="relative z-10">
        ...
        <div className="mb-12 text-center">
          <span className="mb-4 inline-block text-xs font-semibold uppercase tracking-[3px] text-primary font-heading">Mentors</span>
          <h1 className="font-heading text-4xl font-bold text-foreground md:text-6xl">Our Faculty</h1>
          <p className="mx-auto mt-3 max-w-md text-muted-foreground">Guiding and inspiring the next generation of engineers.</p>
        </div>
        ...
        <div className="mx-auto max-w-5xl grid gap-6 md:grid-cols-2">
          {facultyData.map((faculty, index) => (
            <div
              ...
              className="group overflow-hidden rounded-2xl border border-border bg-card transition-all hover:border-primary/20 hover:shadow-glow cursor-pointer active:scale-[0.99]"
            >
              <div className="flex flex-col sm:flex-row">
                ...
                <div className="flex-1 p-6">
                  ...
                  <h3 className="text-xl font-bold text-foreground font-heading">{faculty.name}</h3>
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-muted-foreground line-clamp-2">{faculty.description}</p>
                  <p className="mt-2 text-xs text-primary">Click to view profile →</p>
                </div>
              </div>
            </div>
          ))}
