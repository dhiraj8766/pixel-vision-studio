...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 hidden md:block">
      <div className="flex items-center justify-between pt-5 px-6 lg:px-10">
        {/* Left: Logo + Nav */}
        <div
        ...
        >
          ...
          <div className={`flex items-center gap-1 overflow-hidden transition-all duration-500 ${isOpen ? "max-w-[700px] opacity-100" : "max-w-0 opacity-0"}`}>
            {menuItems.map((item) => (
              <Link key={item.name} to={item.path}
                onMouseEnter={() => handleMouseEnter(item.name)}
                className={`rounded-full px-4 py-2 text-sm font-medium transition-colors whitespace-nowrap ${
                  location.pathname === item.path ? "text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
                }`}>
                {item.name}
              </Link>
            ))}
            <button onClick={() => setIsOpen(false)} className="ml-2 rounded-full p-2 text-muted-foreground transition-colors hover:text-foreground">
        ...
        {/* Right: Social icons */}
        <div className="flex items-center gap-1 rounded-full border border-border bg-card/80 px-3 py-2 backdrop-blur-xl shadow-card">
          {socialLinks.map((s, i) => (
      ...
      {currentDropdown && (
        <div className="absolute left-1/2 top-20 -translate-x-1/2 w-[500px] max-w-[90vw] rounded-2xl border border-border bg-card/95 p-5 backdrop-blur-xl shadow-card animate-[fadeIn_0.2s_ease-out]"
          onMouseEnter={() => clearTimeout(leaveTimeout.current)} onMouseLeave={handleMouseLeave}>
          ...
          <div className="grid grid-cols-2 gap-2">
            {currentDropdown.map((item, i) => (
              <Link key={i} to={item.path} className="rounded-xl p-4 transition-colors hover:bg-muted/50">
