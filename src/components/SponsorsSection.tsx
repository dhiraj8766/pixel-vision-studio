    ...
    <section className="relative bg-background px-6 py-20 md:px-10 lg:px-16 overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        <div className="mb-12 text-center">
          <span className="mb-4 inline-block text-xs font-semibold uppercase tracking-[3px] text-primary font-heading">
            Ecosystem
          </span>
        ...
        <div className="relative overflow-hidden py-8">
          ...
          <div className="flex animate-[marquee_20s_linear_infinite] gap-8">
            {[...sponsors, ...sponsors].map((sponsor, index) => (
              <div
                key={index}
                className="flex h-20 w-44 flex-shrink-0 items-center justify-center rounded-xl border border-border/50 bg-card/50 transition-all hover:border-primary/30 hover:shadow-glow"
              >
                {sponsor.logo ? (
                  <img src={sponsor.logo} alt={sponsor.name} className="h-8 w-auto opacity-40 grayscale transition-all hover:opacity-80 hover:grayscale-0" loading="lazy" />
                ) : (
                  <span className="text-sm font-medium text-muted-foreground/60 hover:text-foreground transition-colors font-heading">
                    {sponsor.name}
                  </span>
                )}
              </div>
        ...
        <div className="mt-12 text-center">
          <div className="inline-flex items-center gap-4 rounded-2xl border border-dashed border-border/50 bg-card/30 px-8 py-5">
